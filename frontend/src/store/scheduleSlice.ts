...
const scheduleSlice = createSlice({
  ...
  extraReducers: (builder) => builder
    ...
    .addCase(fetchSchedules.rejected, (state, action) => { state.status = 'failed'; state.error = action.payload || 'Failed to fetch schedules'; })
    ...
    .addCase(addSchedule.rejected, (state, action) => { state.isAdding = false; state.error = action.payload || 'Failed to create schedule'; })
    ...
    .addCase(updateSchedule.rejected, (state, action) => { state.isUpdating = false; state.error = action.payload || 'Failed to update schedule'; })
    ...
    .addCase(deleteSchedule.rejected, (state, action) => { state.isDeleting = false; state.error = action.payload || 'Failed to delete schedule'; }),
});
