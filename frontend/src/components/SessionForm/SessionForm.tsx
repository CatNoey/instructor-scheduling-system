...
const SessionForm: React.FC<SessionFormProps> = ({ session, scheduleId, scheduleDate, scheduleName, onClose }) => {
  ...
  return <form onSubmit={handleSubmit} className={styles.sessionForm}>
    ...
    <div className={styles.formGroup}><label htmlFor="instructor">세션 표시명</label><input type="text" id="instructor" name="instructor" placeholder="예: 오전 학급 교육" value={formData.instructor} onChange={change} required /><p className={styles.helpText}>강사 지원자가 시간대를 구분할 수 있는 이름을 입력해 주세요.</p></div>
